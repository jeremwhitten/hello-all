...
local HeroIcon = "https://i11.servimg.com/u/f11/16/33/77/19/Nautilus15.png"
local QIcon = "https://i11.servimg.com/u/f11/16/33/77/19/Nautilus10.png"
local PassiveIcon = "https://i11.servimg.com/u/f11/16/33/77/19/Nautilus13.png"
local WIcon = "https://i11.servimg.com/u/f11/16/33/77/19/Nautilus12.png"
local EIcon = "https://i11.servimg.com/u/f11/16/33/77/19/Nautilus11.png"
local RIcon = "https://i11.servimg.com/u/f11/16/33/77/19/Nautilus14.png"
...
local QData =
{
Type = _G.SPELLTYPE_LINE, Delay = 0.25, Radius = 180, Range = 1100, Speed = 2000, Collision = {_G.COLLISION_MINION}
}

local WData =
{
Type = _G.SPELLTYPE_LINE, Delay = math.huge, Radius = 0, Range = 0, Speed = math.huge, Collision = false
}
...
local RData =
{
Type = _G.SPELLTYPE_LINE, Delay = 0.46, Range = 825, Speed = math.huge, Collision = false
}

function Nautilus:LoadMenu()                     
	
--MainMenu
self.Menu = MenuElement({type = MENU, id = "Nautilus", name = "Nautilus Beta.01", leftIcon = HeroIcon})
...
self.Menu:MenuElement({type = MENU, id = "Combo", name = "Combo Mode"})
	...
	self.Menu.AutoR:MenuElement({id = "RxAllies", name = "CountAlliesNear",value = 2, min = 1, max = 5,step = 1})
...
function Nautilus:Draw()
  if myHero.dead then return end
    ...
    Draw.Circle(myHero, 355, 1, Draw.Color(0, 225, 0, 10))
	end
...
if target == nil then return end
	if IsValid(target) then
		...
		if myHero.pos:DistanceTo(target.pos) <= 300 and self.Menu.Combo.UseE:Value() and Ready(_E) then
			...
			local Rdmg = getdmg("R", target, myHero)
			if myHero.pos:DistanceTo(target.pos) <= 825 and self.Menu.Combo.UseR:Value() and Ready(_R) then
...
local target = GetTarget(550)
if target == nil then return end
    if IsValid(target) and myHero.pos:DistanceTo(target.pos) < 850 and Ready(_R) then
        if GetAllyCount (myHero, 850) >= 1 then
